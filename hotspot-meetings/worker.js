...
async function fetchMeetingAssociations(meetingIds) {
  try {
    const path = '/crm/v3/associations/MEETINGS/CONTACTS/batch/read';
    ...
    console.error('Error fetching meeting associations', error);
    return [];
  }
}

async function fetchContactsDetail(contactIds) {
  try {
    console.log('fetch contact details batch');
...
const processMeetings = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    ...
    const meetingAssociations = await fetchMeetingAssociations(meetingIds);

    const meetingAttendees = {};
    for await (const meetingAssociation of meetingAssociations) {
      const meetingId = meetingAssociation.from.id;
      const contactIds = meetingAssociation.to.map((contact) => contact.id);
      const contactsDetail = await fetchContactsDetail(contactIds);
    ...
    data.forEach((meeting) => {
      ...
      const actionTemplate = {
        ...
        meetingProperties: {
          ...
          meeting_attendees_email: meetingAttendees[meeting.id]?.contactsDetail
            .map((contact) => contact.email)
            .join(','),
        },
      };

      if (meeting.associations) {
        console.log('meeting associations', meeting.associations);
      }
...
const pullDataFromHubspot = async () => {
  ...
  for (const account of domain.integrations.hubspot.accounts) {
    ...
    try {
      ...
      console.log(err, {
        apiKey: domain.apiKey,
        metadata: { operation: 'process meetings', hubId: account.hubId },
      });
    }
